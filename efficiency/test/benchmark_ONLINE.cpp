...
int main(int argc, char** argv) {
	...
	logfile_str = "experiments/" + logfile_str + "_benchmark_output.txt";
	...
	time_point<high_resolution_clock> clock_s, clock_e;
	clock_s = high_resolution_clock::now();
	...
	double init_tree = time_from(init_start);
	...
	if (WHICH_ALG==0) {
		cout << "running cpof_dp\n";
		cpof_dp(party, 0.005, TREE_HT, *A, *T, NUM_SAMPLES, NUM_FEATURES, D);
	} else if (WHICH_ALG==1) {
		cout << "running op_cpof_dp\n";
		op_cpof_dp(party, 0.005, TREE_HT, *A, *T, NUM_SAMPLES, NUM_FEATURES, D);
		cout << "init_tree: " << init_tree * 1e-6 << endl;
	} else if (WHICH_ALG==2) {
		cout << "running cpof_eo\n";
		cpof_eo(party, 0.005, TREE_HT, *A, *T, NUM_SAMPLES, NUM_FEATURES, D);
	} else if (WHICH_ALG==3) {
		cout << "running op_cpof_eo\n";
		op_cpof_eo(party, 0.005, TREE_HT, *A, *T, NUM_SAMPLES, NUM_FEATURES, D);
		cout << "init_tree: " << init_tree * 1e-6 << endl;
	} else {
	...
	clock_e = high_resolution_clock::now();

	auto start = time_point_cast<microseconds>(clock_s).time_since_epoch().count();
	auto end = time_point_cast<microseconds>(clock_e).time_since_epoch().count();
